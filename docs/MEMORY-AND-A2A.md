...
> same commit.
> **Document version: 2026-09-04 (rev. 4 — §5.1.6 alpha.28 accepted; rev. 3 — alpha.26/alpha.27; rev. 2 — §6.1 sender side, §5.1.6 alpha.25).** Written from the repository code as of that date; package
...
| alpha.27 | `hex-bez-obyavleniya` **marks instead of locking** (only `obyavlennyj`/`uuid-obyavlennyj`/`strukturnyj:*` lock; the reasoning sits in the code next to `strogo`); the storage refusal reason is non-empty even with an empty `message` (debt 100); the filter file is byte-identical to alpha.26 | **accepted for deployment** 2026-09-04 21:0x as a frozen copy from the sadik disk, subject sum `70c8fb7ee297a7f0` (not in the registry at that moment; the release must produce the same sum); sighted 3/3 (corruption "hex back into the lock set" → the stand turns red), controls 6/6. **Live at Iskra 18:25:06–19:08:16Z, then replaced by alpha.28.** alpha.27 exists neither in the registry nor in git (the number was bumped on disk without a release or a commit); the only artifact is the acceptor's frozen copy with the sum above. Debts for alpha.28: a declared secret whose value contains `-`, `.` or symbols passes silently (`api_key=sk-…`, `password = Xk9#…` → `null`, while `password=Qwerty123` is locked) — first; three stands crash at the recipient without the platform instead of declaring blindness; the three marking-mode rules |
| alpha.28 | two rules in **marking** mode: inflected Russian forms (`ключа:`/`паролем:`/`токена=` → `slabyj:sklonenie`) and the authorization header (`Authorization: Bearer`, `authorization=` → `slabyj:zagolovok`); the dash rule **dropped by measurement** (it marked prose like «ключ - это идентификатор»); `strogo` unchanged | **accepted for `latest`** 2026-09-04 21:5x (subject sum `b7da7abe3741ccfc`); the `latest` tag was moved by Iskra at 19:00Z per the verdict; registry = repository (e8fad2d) = Iskra's disk = **live since 19:08:16Z** (Mikhalych's lift, sums verified before restart, seven of seven) — the four points agree; sighted 2/2 (each rule removed by corruption → its form falls back to `null`), controls 5/5; on the 3599 corpus — +11 marks, no new locks. **The symbol-value hole did not make it in** (fourth edition in a row: `api_key=sk-…` → `null`) — first in alpha.29; three stands still crash at the recipient without the platform instead of declaring blindness. Consumers of `najti_sekret` (export Э8.5, gatekeeper Т2): `slabyj:*` classes are marks, not skips |
...
| restore alpha.18 | `5609aa89b9ccf3ea` (8 files) | **accepted for latest** (2026-09-05 12:3x MSK, acceptance on the registry tarball: canon on disk = tarball, diff 0; on disk 81/81 next to secretary): E8.6 — the last compaction summary is fetched by a separate query (`klass: klassSvodki, skolko: 1`, core orders `sozdano DESC`) and injected whole outside all three cut-offs, only at welcome, exactly one, announced aloud; «not found» and «could not read» are different lines. Auditor's corruption «notice removed» → FAIL on its probe; corruption «old summary instead of the latest» is NOT caught (fixture has a single summary — 80/81, blind 1): LOW, next restore release adds a probe with two summaries where the newest must win. |
| core alpha.40 | `7d585ad81f5bc4d3` (23 files) | **accepted for latest** (2026-09-05 14:05 MSK on the tag tree, npm not yet; 2026-09-05 20:2x MSK npm tarball = tag tree 23/23, diff 0, latest → alpha.40 by the coordinator, clean install verified by the server hand): secret filter — Cyrillic password caught, `process.env.X` false refusal gone (7 forms, 0 defects on live); gatekeeper (approval/request) registered first — dormant by construction (no subscriber returns `ask`); (г) `ochistit` for compaction summaries → next release. |
| secretary alpha.16 | `388dd191094001cd` (18 files) | **accepted for latest, conditionally** (2026-09-05 19:3x MSK, tag tree = disk, stands 20/20 + 5/5 + 10/10 + 8/8 on the tag tree, filter untouched, no default path; in battle since 16:39:43Z; npm tarball = tag 18/18): closes the LOSS of the summary text (rejected summary → 700/600 dir, `putOtklonennyh`) and the ABORT of distillation (debt 114). Does NOT close: stale-summary substitution by restore (item 115), distillation records through the filter (item 113); `chmodSync` after `mkdirSync` and (г) → alpha.17. |
| nudzh alpha.13 | `c04743659757f604` (11 files) | **accepted for latest** (2026-09-05 20:2x MSK, tag tree = disk, stands 24/24 + 6/10/8/8 on the tag tree with deps of the live copy, corruption drops exactly the two probes about printing): path marks to the threshold (`otmetkiPuti`, one line per crossed fraction per cycle) — observation, not protection; the predictor lags one call behind. Not in npm (milestone rule). |
| vyvoz alpha.5 | `171666fdf1eb4e91` (11 files) | **accepted for latest, conditionally** (2026-09-05 21:0x MSK, tag tree = disk, stand 24/24 on the tag tree with the core from the same tree, corruption of the type check drops exactly one): a refusal names WHAT came instead of a path; full circle on a snapshot of the live base 68→68; unknown fields named aloud (`proishozhdenie`, `proverka`). Decision: `proishozhdenie` exported, `proverka` not, with the reason IN the file → alpha.6. |
| core alpha.41 | `e1c2d81c0c27aa3a` (23 files) | **accepted for latest** (2026-09-05 21:2x MSK, tag tree = disk, 12 stands green on the tag tree (dolgovremennoj — legitimate blindness without the secretary beside), private names 0, corruption removing every `seq` mention drops exactly П7-трет 49/50): the refusal names both units — position in CHARACTERS of the record text, source in EVENT NUMBERS (seq) of session.jsonl.zstd. Known open hole: none in this package (rejected knowledge loss is the secretary's → alpha.17). Not in npm (milestone rule). |
| secretary alpha.17 | `d9dcfe2c1e02b2f2` (20 files) | **accepted for latest, conditionally** (2026-09-05 21:4x MSK, tag tree = disk, stands 20/7/7/5/5/14/10/8/9 green on the tag tree with the core beside, private names 0, corruption `mask from the match start` drops exactly the boundary probe 6/7): (г) — declared-secret VALUES in a compaction summary are masked to `***` before the write, the word/form/surrounding text kept, count and classes shouted, original whole in the session journal by seq; rules taken from the core (`najti_sekret` + `zapiraet`), no private copy; a failed mask is said aloud. KNOWN OPEN HOLES (rule 21:12) → alpha.18 REQUIRED before the bundle: rejected distillation KNOWLEDGE is still lost (item 113: same filter, per-item refusal, text saved nowhere); `chmodSync` after `mkdirSync` absent (item 114). Deploy: allowed (better than alpha.16); bundle: only with alpha.18. Not in npm (milestone rule). |
| secretary alpha.18 | `716b1b440e6808a9` (20 files) | **accepted for latest** (2026-09-05 21:5x MSK, tag tree = disk, 9 stands green on the tag tree with the core beside, private names 0; corruption `chmodSync removed` drops exactly the narrowing probe (755 stays 755); corruption `rescue write no-op` drops exactly three: summary saved / directory narrowed / knowledge saved with the refusal re-thrown): closes both holes of alpha.17 — rejected distillation KNOWLEDGE is rescued to `znanie-<epoch>-<pid>.txt` next to `svodka-*.txt` (writer `sozdatZapisatel`, index.js:87/:371), the refusal is re-thrown on purpose so the run keeps the count; directory rights are enforced (`chmodSync` 0o700, then `statSync` re-read, wider than owner → refusal WITHOUT writing; files `wx` 0o600). Known open hole in this package: none. Not proven by event yet (no refusal since 16:39Z). Missing (improvement, not a bug): a success line «summary written, N chars, id» — silence of success still equals absence. Not in npm (milestone rule) — a bundle on it needs a milestone word. |
| vyvoz alpha.6 | `7228a03e860214e5` (12 files) | **accepted for latest, conditionally** (2026-09-05 22:0x MSK, tag tree = disk, stand 33/33 on the tag tree with the core beside, private names 0; corruption `renameSync no-op` drops three probes; wrapper corruption `probe body returns false without throwing` → red 33/34 — the stand wrapper now honours the return): six of seven gates closed — В1 (uzel/otkuda split, zaderzhano in the header, sha256 over record lines, import verifies BEFORE the first write, «no sum = integrity NOT checked»), В2 (identity agent+istochnik+sozdano, pair when no agent, said aloud), В6 (temp+rename beside), В8 (operations journal, its failure neither cancels nor silences), В10 (outside the layout → blindness code 2 before the first probe); core lookup chain name → sibling → own node_modules with the chosen link printed. Open: В9 (config/harness — decision: a node script run by hand → alpha.7). Known open hole: none. Not in npm. |
| vyvoz alpha.7 | `a92af66fb0989e6d` (13 files) | **ACCEPTED for latest** (2026-09-05 22:2x MSK, tag tree = disk, stand 38/38 on the tag tree with the core beside, private names 0; corruption `strict time guard removed` (bin:61) drops exactly В9 «codes split: refusal 1 / blindness 2» 37/38; wrapper corruption `body returns false` → red): all seven gates closed — В1, В2, В6, В8, В9 (a node script run by hand, `bin/vyvoz-pamyati.mjs`, codes 0 done / 1 refusal by subject / 2 instrument blindness; selection --agent/--klass/--s/--po INSIDE the export so the sum and the held count describe the same file; a condition on a missing column → refusal, an empty selection → said aloud, an unparsed time → blindness), В10, core lookup chain. Verified on a copy of the live base (80 records). First accepted version of vyvoz. Pending, not a hole: the two schema fields decision (proishozhdenie exported, proverka not — reason IN the file) → alpha.8. Not in npm — latest → alpha.4 now, this version after the milestone word. |
| core alpha.42 | `aea9e72b2ab4cc79` (23 files) | **accepted CONDITIONALLY for deployment** (2026-09-05 23:3x MSK, tag tree 909b607 = Iskra disk; stands on the tag tree 10/12 green, 2 blind — secretary not alongside; mutation "one-line `return 'deny'` in the handler" → next-probe red; mutation "marker renamed" → red). Subject: six stand wrappers honour the body return value; probe "a non-answering branch must call next"; gatekeeper start line states that chain position is not verifiable. **Diverges from CHANGELOG:** return-blind stands are 11, not 5 — 6 fixed, six still pass on `() => 'deny'` (krik-zvuchit, readme, sborki, filtra-vhoda, ocheredi-dostavki, dolgovremennoj; probe inserted before `process.exit`, exit 0). **Hole in the next-probe:** bare `return;` is not caught. Debts for alpha.43: six wrappers, bare return, CHANGELOG "five" → measured count. Iskra's live process at verdict time is not alpha.42 (started 16:39Z) |
| core alpha.43 | `1dd65d25b9c0e784` (24 files) | **accepted for deployment** (2026-09-06 00:4x MSK, tag tree 9bc15b4 = disk; stands 11/13, 2 blind — secretary not alongside; mutations "drop reason check" / "UPDATE touches origin" / "migration without column guard" turn exactly their own probes В2/В9/В1 red; a copy of the live base (92 rows) opened twice by the tag code — columns present, touches 0, NULL 92, boundary 68 untouched). Subject: Э8.3 П1 — kasanij/poslednee_kasanie migration in one transaction, otmetitKasanie with the single reason "vydacha-agentu", a failed write does not break delivery. Debts: nothing calls the method yet (live counter stays 0 until the budget selector calls it, dedup "once per turn" lives there too); В4–В7 in budget/export one at a time; alpha.42 debts (six wrappers, bare return) untouched |
| secretary alpha.19 | `91319e2c1fba0629` (21 files) | **accepted** (2026-09-06 01:5x MSK, B1/#133: tag 23307ea = disk; any model refusal — no text block or empty text — writes a task zadanie-<time> with topic/slice/outcome/reason next to rejected items; no auto-retry by design; stands 10/10, mutations red) |
| core alpha.45 | `0f3dae7722f67fc2` (24 files) | **accepted for deployment** (2026-09-06 01:5x MSK, tag e5433cf = disk; id dedup in otmetitKasanie + forwarding from the service; stands 11/13; mutation 'drop Set' → 2 red; covers alpha.44) |
| restore alpha.22 | `5249797b4de3766b` (8 files) | **accepted for deployment** (2026-09-06 01:5x MSK, tag b4bb590 = disk; touches on delivery with three spoken branches, summary id without duplicates, fixture memory in core order; stands 92/92; mutation → 5 red; covers alpha.19–21). ⚠️ live #115 cases not yet explained |
| byudzhet alpha.9 | `b170d1d49bb064d2` (12 files) | **accepted for deployment** (2026-09-06 01:5x MSK, tag 61fe024 = disk; warmth instrument reads sozdano and kogda, three touch inputs as absolute numbers, weight 0 stated; stands green; mutation red) |
| secretary alpha.20 | `113437f30b7b8b1d` (22 files) | **accepted for deployment** (2026-09-06 02:3x MSK, debt 87: tag a016610; the discarded BEGINNING of a truncated transcript is handed to an `otbroshennoe` handler and saved as nachalo-sreza-* next to rejected items with a header naming slice, sizes, share and 'this knowledge did NOT reach memory'; no handler → spoken cry, not silence; stands 10 green (mask stand blind in my /tmp copy — my environment, 7/7 on disk); mutations 'handler not called' → 2/6, 'header without the line' → 5/6; superseded on disk by alpha.21 within 10 minutes) |
| secretary alpha.21 | `51d199020039c9b6` (23 files) | **accepted for deployment** (2026-09-06 02:3x MSK, debt 92: tag cfa2b44 = disk, BOUNDARY: disk state at 23:24:44Z; article length limit is parsed from the STATYA prompt (not hard-coded; not parsed → blindness cry about the instrument), overruns and max length are counted on output and printed as 'a measurement, not a guard' — nothing is cut, prompt unchanged; 12/12 stands on disk, predela-stati 6/6; mutations 'overruns not counted' → 3/6, 'limit not parsed' → 1/6 naming the SOURCE of the number, not the boundary. Lesson: gates on a live tree have an expiry equal to the time to the author's next release — verdicts now name a boundary (sum + files + time), and the deployer re-checks it with one command right before restarting) |
| core alpha.46 | `d495e16dfa9f504b` (24 files) | **accepted for deployment** (2026-09-06 02:5x MSK, tag f1793e9; text-only: a farm member's name in a stend-privratnika comment replaced by a role, CHANGELOG; no code touched; private-names probe on the tag tree: alpha.45 → exit 1 'do not publish', alpha.46 → exit 0 clean; 13 stands: 11 green, 2 blind in my /tmp copy (dolgovremennoj, privratnika — environment, same as alpha.43). NOT the #66 fix (value vs quotation in the input filter) — that stays open. BOUNDARY: at 23:46:18Z the disk (838e2b18be32118e) already differed from the tag — WIP rename t→proba in three stands; the deployer's boundary check must stop until the next tag) |
| core alpha.47 | `ce760396ebcb55ab` (24 files) | **accepted for deployment** (2026-09-06 02:5x MSK, tag fba140d = disk, BOUNDARY: disk state at 23:53:53Z; debt 120: one runner name `proba(` in all 13 stands (three used `t(`); diff beyond the rename is version + CHANGELOG only, no code; stands on the tag copy 11 green + 2 blind (my /tmp environment); mutation 'one probe under the old name' → ReferenceError, stand exits 1 — loud, not silent (Iskra re-measured the debt's cost: not 'one probe skipped' but 'stand not run'); private-names probe exit 0. Not the #66 fix) |
| vyvoz alpha.8 | `99363ce3ac25d424` (13 files) | **accepted** (2026-09-06 03:5x MSK, tag 6cd1832 = tree = disk; E8.5 per plan v4: `proishozhdenie` is exported (a property of the record), `proverka` is not, with the reason in the file (the check mark refers to THIS machine's journal); imported records read an empty origin as 'not established', not 'pre-protocol'; stands 40/40 with the sibling core from the same tag; mutations 'drop proishozhdenie' → 39/40 and 'export proverka' → 39/40, control 40/40; verified by the author on a copy of the live DB: 104 records. Not mounted (absent from the profile), no effect on deployment; registry latest stays a4-bound, alpha.8 not published before a milestone) |
| secretary alpha.22 | `97dafa4495ac1e14` (24 files) | **accepted for deployment** (2026-09-06 06:4x MSK, tag 32f0e13 = disk, BOUNDARY 02:46:37Z; fifth cause of lost knowledge: the model answers the topic-selection call with PROSE instead of a JSON array — the raw answer is now saved as proza-* next to rejected items with the reason, the format instruction is repeated at the END of the user message; stands 12 green + prose stand 5/5; mutation 'prose not saved' → 3/5, control 5/5. Not proven: that the live model answers in prose less often after the repeat — only our behaviour is tested) |
| core alpha.48 | `ead1e65987f3a973` (25 files) | **accepted for deployment** (2026-09-06 07:1x MSK, tag 055ba15 = disk; B1/B4 in one release: the sample mask moved into the core onto the write path (scope klassyZnaniy ∪ svodka*, journal maska-obrazcov / maska-ne-primenima); live probe with production code: 6 of 6 saved rejected texts pass the filter after masking, structural sk- is still rejected; stands from the production layout 12/13 green, privratnik blindness named; in production since 04:19:39Z) |
| secretary alpha.23 | `ccbb77274af699ba` (23 files) | **accepted for deployment** (2026-09-06 07:1x MSK, tag 055ba15 = disk; core import removed — 0 code references to dsh-pamyat-core (two are comments), the mask moved to the core, item 188 closed by construction for any layout; stands green from the production layout; in production since 04:19:39Z) |
| restore alpha.23 | `62bc3d86f58f63d9` (8 files) | **accepted for deployment** (2026-09-06 08:2x MSK, tag 15afc5f = disk; B2/item 115: post-compaction restore checks the summary age against the compaction time — the warning «summary is NOT from this compaction, written N min before it» goes into the injected text itself, the third outcome «age could not be verified» is separated; stands from the production layout 98/98, mutation «empty header» → 96/98 + 1 blindness; production still runs a22 — applied by the next deployment) |
| core alpha.49 | `02b3d8ca5ce03f3f` (25 files) | **accepted for deployment** (2026-09-06 08:4x MSK, tag 451afe9 = disk; P26 «axes of the predicate»: field-access narrowed by access root — `password: Secret.Pass` is now blocked, `process.env.X`/`config.token`/`this.secret.value` pass; stands in the production layout 12 green / 1 blindness (gatekeeper), mutation «empty roots» → red; live corpus 127 records: 1 hit (id 56, slabyj) — same as a48. OPEN R4: a value whose dot-separated pieces are all shorter than 6 chars with fewer than 3 alphabet classes (`api_key: abc.def.ghi`, `password: Foo.bar`) passes silently — same class as the «length ladder» of 04.09) |
| restore alpha.24 | `328ca28572e69e93` (8 files) | **accepted for deployment** (2026-09-06 08:5x MSK, tag 2fad5a4 = disk; B2: the freshness boundary is the START of the same compactionId, not its end; on a23 the production sign turned out FALSE — the fresh summary of 05:36:44Z (record 127) was injected with the header «OLDER by 0 min», because the summary is written on summary before end; stand 101/101 in the production layout; mutation «boundary = end» → 2 red, «id check removed» → 1 red; production still runs a23 — applied by the next deployment; production sign for a24 — first compaction after deployment: «summary of this compaction») |
| core alpha.50 | `463c95251875e569` (25 files) | **accepted for deployment** (2026-09-06 09:1x MSK, tag 94d7ea5 = disk; alphabet classes are counted by letter properties (\p{Ll}, \p{Lu}\p{Lt}, \p{Lo}\p{Lm}, \p{N}, other) instead of a range list — declared secrets in Armenian/Greek/Georgian/Hebrew/Arabic/CJK are now blocked (passed silently on a49); Latin/Cyrillic/field references unchanged; stands in the production layout 12 green / 1 blindness; corpus of 138 records: 1 hit (id 56) — same as a49. OPEN: P26-bis (candidate by [A-Za-z0-9_]) and declared values shorter than 8 chars; R6 — «Unrecognized key: lastTouchedAt» is blocked falsely on a49 and a50 (not a regression)) |
| core alpha.51 | `3a4c3e69b1f5304a` (25 files) | **accepted for deployment** (2026-09-06 10:4x MSK, tag e8cde28 on origin = disk; text only: machine-private paths in stand comments korpus-progon.mjs anonymised — caught by the new milestone gate in the release wrapper; 0 non-comment changes; 0 private paths left in the package; stands 12 green / 1 blindness; «version bump before edit» order observed — first instance) |
| core alpha.53 | `5d98993266729d9e` (25 files) | **accepted for deployment** (2026-09-06 11:2x MSK, tag 81fc9eb on origin = disk; Z1: the gatekeeper summary is printed on the first request and every tenth (constant ITOG_KAZHDYE) — otherwise on SIGKILL/OOM the gatekeeper's participation is unprovable; Z3: README section «gatekeeper = no human nearby»; a52 in passing — the stand no longer blames the package for a foreign platform-dictionary break; stands 12 green / 1 blindness, gatekeeper stand 16/16; mutations «print disabled» → 2 red, «print always» → exactly Z1-bis; «version bump before edit» order NOT observed — second instance) |
...
| core alpha.31 | `4c20c5de4b971283` (20 files) | **accepted for latest** (2026-09-05 02:54 MSK, acceptance on the registry tarball 26 s after the 23:53:55Z release): group B — declared values with special characters (`password = Xk9#mQ2$vL8p` etc.) → obyavlennyj, 4/4; exported `rezhim(klass)` → zapiraet/pomechaet/neizvesten (table checked over 12 classes) with `zapiraet` as a wrapper; probe P23 (the table knows its own classes) and the single-source-of-truth stand; stands 10/10 with secretary a15 beside; corpus 3599 — 201 = 201 (0 new false positives); sighted controls 6/6; private names 0. Still open: the pre-existing false positive on `key: lastTouchedAt` (not this change) |
| core alpha.32 | `4d6ea232c93c452f` (21 files) | **accepted for latest** (2026-09-05 10:5x MSK, acceptance on the registry tarball: canon sum on disk = tarball, diff 0, stend-rubezha 15/15 from the tarball, commit 0c02450 in origin): E5.3 origin milestone — `nastrojki` table, one `BEGIN IMMEDIATE` migration for `proishozhdenie` + `proverka`, milestone = `MAX(id)` set only if absent, laundering below the milestone refused (`PAMYAT_ZAPIS_DO_PROTOKOLA`); verified on a read-only snapshot of the live base (55 records → milestone 55/55, all 55 «pre-protocol», second pass unchanged). Non-blocking LOW: the stand is blind to id gaps (`MAX(id)`→`COUNT(*)` corruption passes 15/15) — probe V5-bis due in the next core release. |
| core alpha.33 | `d55b9e0513db85f3` (21 files) | **accepted for latest** (2026-09-05 11:2x MSK, acceptance on the registry tarball: canon on disk = tarball, diff 0, stend-rubezha 16/16 from the tarball; on disk all 11 stands green): V5-bis — milestone on a base with id gaps = `MAX(id)`, not `COUNT(*)`; the probe guards its own validity (fails as «probe unfit» when there are no gaps); the auditor's `MAX(id)`→`COUNT(*)` corruption now reddens exactly V5-bis (15/16, «milestone 4 at MAX(id)=10»). LOW, not blocking: three stands (dolgovremennoj, filtra-vhoda, ocheredi-dostavki) throw `ERR_MODULE_NOT_FOUND @deepseek-ai/cordis` outside the platform instead of reporting blindness by code. |
| core alpha.34 | `14797db44ab237cf` (21 files) | **accepted for latest** (2026-09-05 11:3x MSK, acceptance on the registry tarball: canon on disk = tarball, diff 0; from the tarball in a clean location: 11 stands, 0 mismatches, 5 blind by code; on disk 11/11, stend-rubezha 16/16): blindness outside the platform — the three stands that threw `ERR_MODULE_NOT_FOUND @deepseek-ai/cordis` now import dynamically and report «СЛЕПОТА: платформа не установлена» instead of failing (closes the LOW from alpha.33). |
| core alpha.35 | `50cf9ff13c69602c` (21 files) | **accepted for latest** (2026-09-05 11:5x MSK, acceptance on the registry tarball: canon on disk = tarball, diff 0; clean location 11 stands / 0 mismatches / 5 blind by code; on disk 11/11; stend-rubezha 23/23 from the tarball): E8.1 second half — writer for `proverka`: `statusProverki` returns three states as three strings (`est` / `utrachena` / `ne-proveryalos`) plus `otmetka-negodna` for an unparsable mark; `otmetitProverku` refuses any outcome other than est|utrachena (`PAMYAT_ISHOD_SVERKI_NEGODEN`) and refuses a mark without the instrument name (`PAMYAT_SVERKA_BEZ_PRIBORA`). Auditor's three corruptions (absence read as loss → 21/23; instrument optional → 22/23; broken mark read as unchecked → 22/23) each redden their own probe. |
| core alpha.36 | `20188cad17ace31c` (21 files) | **accepted for latest** (2026-09-05 12:1x MSK, acceptance on the registry tarball: canon on disk = tarball, diff 0; `src/` unchanged since alpha.35; clean location 11 stands / 0 mismatches / 5 blind by code; on disk 11/11, stend-filtra-vhoda 45/45): async-body protection in the last stand without it — the synchronous runner now refuses a probe whose body returns a promise. Auditor's corruption (async probe that throws) → FAIL on alpha.36; the same corruption passes green on alpha.35's runner (control) — the defect was real. |
