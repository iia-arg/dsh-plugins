...
export function otkrytHranilishche(putK, { drajver } = {}) {
  ...
  return {
    ...
    otmetitKasanie({ ids, povod, kogda = Date.now() }) {
      ...
      // 🔴 КАСАНИЕ СЧИТАЕТСЯ ОДИН РАЗ ЗА ХОД, сколько бы раз запись ни встретилась в выдаче
      // (уточнение ворот В2, приёмка 05.09.2026). Повтор внутри одного хода грел бы запись
      // вдвое, и это не теория: в брифинге сводка компакта добавляется ОТДЕЛЬНО от отбора,
      // и попади она заодно в отобранные — тот же id пришёл бы дважды. Сегодня не приходил
      // (сводка id=56, отобраны 62…68), но «сегодня не совпало» — не устройство.
      // Дедупликация стоит ЗДЕСЬ, а не только у зовущего: правило про предмет обязано
      // держаться независимо от того, кто и как собрал список.
      const spisok = [...new Set((Array.isArray(ids) ? ids : [ids])
