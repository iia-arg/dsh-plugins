      ...
      if (sekret) {
        ...
        if (strogo) {
          zhurnal?.otmetit({
            ...
            // 🔴 ОБЕЩАНИЕ «ВОССТАНОВИМО» ОБЯЗАНО НАЗЫВАТЬ ОТСЧЁТ (05.09.2026).
            // Прежняя строка говорила «восстановимо по источнику: срез журнала
            // append-only» — и разбор упёрся ровно здесь: читающий принял «#62067-70176»
            // за БАЙТЫ, не нашёл их в сжатом файле и остановился, а позиция 8822 в
            // 8109 «байт» не помещалась. Единица в ссылке — НОМЕРА СОБЫТИЙ (seq), но
            // сама ссылка об этом молчала.
            // Класс: обещание восстановимости сильнее того, что ссылка реально даёт;
            // «восстановимо» без указания отсчёта читается как гарантия, а гарантии нет.
            // Позиция считается в ЗНАКАХ текста записи — это третья единица рядом, и её
            // тоже надо назвать, иначе её примут за смещение в файле.
            pochemu: 'в тексте найден секрет класса «' + sekret.klass + '» на позиции ' +
                     sekret.pozicia + ' (позиция — в ЗНАКАХ текста записи, не в файле); ' +
                     'значение не печатается. Знание восстановимо: источник ниже — ' +
                     'НОМЕРА СОБЫТИЙ (seq) в session.jsonl.zstd каталога этой сессии, ' +
                     'не байты и не строки. Распаковать целиком и отобрать по полю seq',
            istochnik: zapis.istochnik ?? null,
          });
