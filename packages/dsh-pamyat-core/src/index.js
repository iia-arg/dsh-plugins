  ...
  ctx.provide('pamyat', {
    ...
    zapisat(zapis) {
      ...
      if (sekret) {
        ...
        // 🔴 ОСНОВАНИЕ ВЫБОРА РЕЖИМА — ОДНОЗНАЧНОСТЬ ПРИЗНАКА, а не «намерение против вида».
        // Прежняя формулировка («автор назвал сам → запирать, вид строки → помечать») не
        // разделяет: структурные признаки читают ВИД строки и запирают совершенно законно,
        // потому что заголовок PEM не бывает ничем иным. Работает другое: запирать можно
        // тот признак, который НЕ БЫВАЕТ ничем, кроме секрета.
        //
        // 🔴 ПОЭТОМУ hex-bez-obyavleniya ПОМЕЧАЕТ, а не запирает — с 0.1.0-alpha.27.
        // В его собственном комментарии в filtr-vhoda.js написано дословно: «хеш и ключ
        // неотличимы по виду». Класс сам объявил свою неоднозначность — и до этой правки
        // всё равно стоял среди запирающих, единственный такой в списке.
        // ⚠️ И решающее, чтобы не переиграли обратно по числу: на живом корпусе 3661 записи
        // этот класс дал НОЛЬ срабатываний, достижимость проверена подставными пробами
