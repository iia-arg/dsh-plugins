...
proba('🔴 сказано, что итог — НИЖНЯЯ оценка, а не расход', () => {
  if (!/НЕ МЕНЬШЕ/.test(ru)) throw new Error('русский не называет нижнюю оценку');
  if (!/LOWER BOUND|lower bound/i.test(en)) throw new Error('английский не называет');
});
