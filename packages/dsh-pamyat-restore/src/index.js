...
export function perevesti(zapis) {
  ...
  return {
    ...
    // 🔴 КАСАНИЯ ЕДУТ ДАЛЬШЕ (Э8.3 П1, ворота В6). Без них прибор тепла у бюджета видит
    // только возраст — то есть считает свежесть под именем тепла. Три входа обязаны быть
    // видны там, где тепло печатается, иначе «тепло» останется словом.
    // ⚠️ Едут КАК ЕСТЬ, включая ноль и null: ноль значит «касаний не было с появления
    // счётчика», null — «поля нет вовсе, ядро старше alpha.43». Подставить сюда ноль
    // значило бы стереть это различение ровно на переходе — та же беда, что была с верой.
    kasanij: zapis.kasanij,
    poslednee_kasanie: zapis.poslednee_kasanie,
  }
}
  ...
  ctx.on('agent/pre-step', async ({ agent, turn, step, signal }, next) => {
    ...
    if (config.welcomeEnabled && !welcomeDano.has(sid)) {
      ...
      if (briefing || svodkaCelikom) {
        ...
        // 🔴 КАСАНИЕ ОТМЕЧАЕТСЯ ЗДЕСЬ — ЭТО И ЕСТЬ ВЫДАЧА АГЕНТУ (Э8.3 П1, ворота В2).
        // Счётчик касаний завёлся в ядре alpha.43 и до этой строки был МЁРТВ: поля есть,
        // писать их некому. «Касаний ноль» означало бы разом «не выдавали» и «никто не
        // отмечает» — а это разные вещи, и вторая чинится, первая нет.
        //
        // 🔴 ЗАЧЕМ ЭТО НУЖНО РАНЬШЕ ВЕСА. Вес прибавки за касание равен нулю и включится
        // только по замеру (ворота В4). Но замерять нечего, пока никто не касается: след
        // выдач сегодня живёт ТОЛЬКО в журнале службы, а он ротируется — за всю его
        // глубину сохранилось две выдачи. Касания кладут тот же след В БАЗУ, где он
        // переживает ротацию, и делают замер возможным.
        //
        // ⚠️ ГРАНИЦА, названная здесь: отмечаем ПЕРЕД inject. Если вставка не состоится,
        // касание останется записанным. Обратный порядок невозможен — после return сюда
        // возврата нет; цена ошибки — счётчик, а не выдача.
        try {
          const p = pamyatIz(ctx)
          if (p && typeof p.otmetitKasanie === 'function' && idsB.length) {
            const itog = p.otmetitKasanie({ ids: idsB, povod: 'vydacha-agentu' })
            if (itog?.otkaz) {
              say(`касания НЕ записаны (${itog.otkaz.code}): ${String(itog.otkaz.pochemu).slice(0, 120)}. ` +
                  'Выдача состоялась и не отменяется; счёт касаний за этот брифинг потерян.')
            } else {
              say(`касания отмечены: ${itog?.otmecheno ?? 0} из ${idsB.length} выданных записей`)
            }
          } else if (p && typeof p.otmetitKasanie !== 'function') {
            // Ядро старше alpha.44 — метода нет. Это НЕ отказ выдачи и не молчание:
            // счётчик касаний просто не ведётся, и знать об этом надо, иначе нули в
            // базе прочтутся как «записи холодные».
            say('касания НЕ отмечаются: у службы памяти нет otmetitKasanie (ядро старше alpha.44). ' +
                'Нули в счётчике означают «некому отмечать», а не «записи не нужны».')
          }
        } catch (e) {
          say(`касания не отмечены: ${String(e?.message ?? e).slice(0, 120)}`)
        }
