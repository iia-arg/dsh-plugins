...
let apply, Config, decide, perevesti, renderWelcome
try {
  ({ apply, Config, decide, perevesti, renderWelcome } = await import('../src/index.js'))
} catch (e) {
...
// ═══ Б2: СВОДКА СТАРШЕ КОМПАКТА НАЗЫВАЕТСЯ ВСЛУХ, И ИМЕННО ВО ВСТАВЛЯЕМОМ ТЕКСТЕ ═══
// 🔴 ПАРА, А НЕ ОДНА ПРОБА. Молчаливая подстановка прежней сводки хуже потери: потеря
// заметна отсутствием, подмена не заметна ничем — та же форма, та же длина, то же имя.
// Одна проба «предупреждение есть» доказала бы половину: если оно печатается ВСЕГДА, оно
...
// --- ПОМЕТКА «НЕ ПОДТВЕРЖДЕНО ЧЕЛОВЕКОМ» В ТЕКСТЕ БРИФИНГА (06.09.2026) ------
// Условие координатора в редакции после вопроса соседа: флаг=1 -> пометка в ТЕКСТЕ,
// флаг=0 -> МОЛЧАНИЕ. Пробы парой намеренно: одна половина без другой узаконила
// бы фон — «пометка есть всегда» прошло бы первой пробой как успех.
{
  const cfgW = { welcomeBudget: 4000, ignoreAfterMs: 30 * 86400000, useVeraThreshold: 0.7 }
  const svezho = Date.now() - 3600000
  const bazovaya = { id: 1, klass: 'ogranichenie', soderzhim: 'СОДЕРЖИМОЕ-МЕТКА',
    agent: 'vladelec-znaniya', istochnik: 'sekretar', sozdano: svezho, vera: 0.9 }

  const sFlagom = renderWelcome([perevesti({ ...bazovaya, bez_podtverzhdeniya: 1 })], cfgW)
  t('пометка: при флаге 1 сказана в САМОМ тексте брифинга',
    typeof sFlagom === 'string' && sFlagom.includes('не подтверждено человеком'), String(sFlagom))
  // 🔴 Причина названа замером: флаг ставится ТОЛЬКО когда на узле нет отвечающего.
  t('пометка: названа причина, а не приписан источник',
    typeof sFlagom === 'string' && sFlagom.includes('отвечающего на узле не было')
      && !sFlagom.includes('из дистилляции'), String(sFlagom))
  // Дата — записи, а не сегодняшняя: пометка про то, КОГДА записали без подтверждения.
  t('пометка: дата — дата записи',
    typeof sFlagom === 'string' && sFlagom.includes(new Date(svezho).toISOString().slice(0, 10)),
    String(sFlagom))

  // ВТОРАЯ ПОЛОВИНА ПАРЫ. Ноль НЕ значит «человек подписал» — он значит «события
  // не было». Любое слово о подтверждении при нуле есть ложная подпись.
  const bezFlaga = renderWelcome([perevesti({ ...bazovaya, bez_podtverzhdeniya: 0 })], cfgW)
  t('пометка: при флаге 0 о подтверждении НИ СЛОВА',
    typeof bezFlaga === 'string' && !/подтвержд/i.test(bezFlaga), String(bezFlaga))
  t('пометка: при флаге 0 запись всё равно в брифинге (молчание, а не пропуск)',
    typeof bezFlaga === 'string' && bezFlaga.includes('СОДЕРЖИМОЕ-МЕТКА'), String(bezFlaga))

  // Время записи неизвестно — дату не выдумываем.
  const bezVremeni = renderWelcome([perevesti({ ...bazovaya, sozdano: null, bez_podtverzhdeniya: 1 })], cfgW)
  t('пометка: без времени записи дата НЕ выдумана',
    typeof bezVremeni === 'string' && bezVremeni.includes('дата записи неизвестна'), String(bezVremeni))
}

const ZHDYOM = 107  // 06.09.2026: 101 -> 107, шесть проб пометки «не подтверждено человеком» (пара: пометка при флаге · при нуле НИ СЛОВА о подтверждении; плюс запись при нуле не пропадает, причина названа замером, дата — записи, без времени дата не выдумана). Прежде: 98 -> 101, три пробы рубежа свежести (без compaction/start — «сверить не удалось», начало чужого компакта не годится, «сводка этого сжатия» сказана вслух); форма подставных событий приведена к живой: start -> summary -> end. Прежде: 93 -> 98, пять проб Б2 «сводка старше компакта» (пара: старая называется числом минут во ВСТАВЛЕННОМ тексте · на свежей предупреждения НЕТ · без времени — «сверить не удалось»). Прежде: 05.09.2026: 74 -> 81, семь проб Э8.6; 81 -> 90, девять проб Э8.3 П1; 90 -> 93, три пробы «из двух сводок поднимается новейшая» (находка приёмки: [0] и [последняя] совпадали на фикстуре с одной сводкой) (касание на выдаче: повод, состав, число, отказ, старое ядро, исключение) (сводка целиком вне бюджета + зеркальная сторона «сводки нет»)
