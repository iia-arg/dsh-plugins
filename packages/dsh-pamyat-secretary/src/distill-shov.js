...
export async function distillirovat({ putZhurnala, dannye, seansId, nastrojka, krik, zapisat, zadanie, otbroshennoe, sohranitProzu }) {
  ...
  // 🔴 ПРЕДЕЛ ДЛИНЫ СТАТЬИ МЕРИТСЯ НА ВЫХОДЕ, А НЕ СЧИТАЕТСЯ ВЫПОЛНЕННЫМ (долг 92).
  // Замер по живой базе 06.09.2026: 45 записей из 92 длиннее предела — 49%, и доля та же,
  // что была 04.09 (6 из 12). То есть это не разовый промах модели, а устойчивое свойство.
  // Медиана ровно 1000: модель предел ВИДИТ и целится в него, но промахивается вверх у
  // половины статей. Требование, написанное в промпте заглавными, требованием не является —
  // его исполнение есть свойство чужой модели, и у нас нет власти, кроме проверки на выходе.
  // ЧЕГО ЭТО НЕ ДЕЛАЕТ: не режет, не переспрашивает, не меняет промпт. Выбор лечения —
  // предмет автора замысла (а: переспрос, дорого при 49%; б: резать, теряется конец мысли;
  // в: объявить норму; г: понизить объявленный предел). Здесь только число, чтобы решать
  // было по чему.
  // ПРЕДЕЛ БЕРЁТСЯ ИЗ САМОГО ПРОМПТА, а не зашит рядом: два числа разошлись бы молча при
  // первой же правке текста. Не вынулся — говорим СЛЕПОТУ, а не подставляем умолчание.
  const m_predel = /не длиннее (\d+) знаков/.exec(STATYA);
  const PREDEL_STATI = m_predel ? Number(m_predel[1]) : null;
  if (!PREDEL_STATI) krik('⚠️ предел длины статьи НЕ ВЫНУТ из промпта: длину мерить не с чем. Это про прибор, не про статьи.');
  let dlinnyh = 0, maxDlina = 0;
  ...
  for (const t of temy) {
    ...
    try {
      const telo = s2.tekst.trim();
      zapisat({ klass, soderzhim: telo, istochnik });
      itog.zapisano++;
      if (PREDEL_STATI && telo.length > PREDEL_STATI) {
        dlinnyh++;
        if (telo.length > maxDlina) maxDlina = telo.length;
      }
  ...
  itog.dlinnyh = dlinnyh;
  itog.predelStati = PREDEL_STATI;
  if (PREDEL_STATI) {
    krik(itog.zapisano === 0
      ? `предел статьи ${PREDEL_STATI} знаков: записанных статей нет — мерить нечего`
      : `предел статьи ${PREDEL_STATI} знаков: превысили ${dlinnyh} из ${itog.zapisano}`
        + (dlinnyh ? `, наибольшая ${maxDlina}` : '')
        + '. Статьи ЗАПИСАНЫ как есть: предел не режет и не переспрашивает — это замер, не защита.');
  }
  itog.rashod = rashod;
