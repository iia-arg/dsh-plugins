 ...
 * @param {string} o.otkuda    как назвать источник в заголовке — ОБЯЗАТЕЛЕН, умолчания нет
 * @param {string} [o.yadro]   путь к модулю фильтра (для стендов)
 * @param {object} [o.otbor]   что брать: {agent, klass, s, po} — время в мс epoch
 */
export async function vyvezti({ baza, fajl, otkuda, yadro, otbor = {}, krik = console.error }) {
  // Договор проверяется ДО открытия базы: не смогли — файла не будет вовсе.
  const filtr = await vzyat_filtr(yadro);

  // 🔴 «ОТКУДА» ОБЯЗАТЕЛЕН, УМОЛЧАНИЯ НЕТ (решение координатора 06.09.2026, условие
  // автора предмета там же). Прежнее умолчание «неизвестно» вписывало в файл значение,
  // за которое никто не отвечал, — и на той стороне оно бесполезно ровно тому, ради кого
  // поле заведено: принимающий не знает, чья это память, а спросить уже некого.
  // Отказ стоит ЗДЕСЬ, а не в скрипте: библиотеку зовут программы, и файл от этого
  // уезжает так же. Проверка в одном месте на оба входа.
  // ⚠️ ПОРЯДОК НАМЕРЕННЫЙ: договор ядра проверяется РАНЬШЕ. Договор ядра — про
  // безопасность (нечем отличить секрет), «откуда» — про пригодность файла на той
  // стороне. Безопасность вперёд; и заодно пробы на отказ ядра продолжают мерить
  // отказ ядра, а не отсутствие «откуда».
  if (typeof otkuda !== 'string' || !otkuda.trim()) {
    const e = new Error(
      'не назван источник: «откуда». Умолчания нет намеренно — подставленное значение '
      + 'уехало бы в файл как названное, и принимающая сторона прочла бы его как данные. '
      + 'Назовите источник явно (в скрипте: --otkuda <имя>)');
    e.code = 'VYVOZ_NET_OTKUDA';
    throw e;
  }
  ...
  const shapka = {
    ...zagolovok({
      otkuda,
      // 🔴 ЗДЕСЬ ЗАМЕР, А НЕ УМОЛЧАНИЕ — И РАЗНИЦУ НАДО ДЕРЖАТЬ, ЧТОБЫ СЛЕДУЮЩИЙ НЕ
      // «ПОЧИНИЛ» ЭТУ СТРОКУ (долг 128, 06.09.2026).
      //   УМОЛЧАНИЕ — подставить значение, которого НЕ ЗНАЕШЬ, чтобы поле не пустовало.
      //   ЗАМЕР     — снять факт командой в момент действия.
      // Признак различения: можно ли снять значение командой в этот самый миг. `otkuda`
      // снять нечем — его знает только вызывающий, поэтому там отказ. `uzel` снимается
      // ОС здесь и сейчас, и это факт о том, ГДЕ файл сделан.
      // ⚠️ И сторона операции важна не меньше (поправка автора предмета): замер на
      // ЧИТАЮЩЕЙ стороне вёл бы себя как умолчание — он отвечал бы про чужой узел при
      // каждом чтении. Здесь сторона ПИШУЩАЯ, значение снимается ОДИН раз и переносом
      // файла не переписывается.
      // ⚠️ ЧЕГО ЭТО НЕ ЗАКРЫВАЕТ: в контейнере имя узла может быть не тем, что ожидает
      // читающий. Отдельный вопрос, здесь не решается.
      uzel: hostname(),
  ...
  return {
    ...
    // Из ШАПКИ, а не вторым вызовом hostname(): отчёт остаётся у вывозящего, файл уезжает,
    // и два вычисления одной величины разошлись бы молча. Одно значение — один источник.
    uzel: shapka.uzel,
    summa: shapka.summa,
    fajl: fajl ?? null,
  };
