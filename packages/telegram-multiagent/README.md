...
### Случай, на котором это выяснилось (имя, версия, дата — чтобы не читалось как общее место)

05.09.2026 метка `neprinyato` у этого пакета стояла на версии **1.5.5**, а 1.5.5 была
**принята** 04.09.2026 в 21:13 (сумма предмета `a0df8c9d3bdb0c5f`, вердикт в дереве §1.3).
То есть реестр говорил два противоположных слова об одном тарболе, и всякий, кто читал метку
как ответ о приёмке, был бы обманут.

Снять метку нечем: у нашего ключа нет права на удаление (403 на `dist-tag rm`), и второго
ключа не будет. Но у указателя есть третье состояние помимо «стоит» и «снят» — **переведён**:
06.09.2026 метка переведена на **1.5.2**, версию, отклонённую вердиктом. Теперь она говорит
правду, и говорит её самостоятельно — без знания наших внутренних правил.

⚠️ Отсюда общее: метка может отстать от вердикта в любой момент, потому что её двигает рука,
а не приёмка. Сверять — с вердиктом.
