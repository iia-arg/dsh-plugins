...
export function apply(ctx, config = {}) {
  ...
  // 🔴 СЛИЯНИЕ КАНАЛОВ В ОДНУ ПАМЯТЬ (20.08.2026, решение владельца).
  ...
  // 🔴 КОМУ АДРЕСОВАН ОТВЕТ (21.08.2026, задача владельца).
  ...
  // ── НАСТРОЙКА ДОСТАВКИ, ОТДЕЛЬНЫМ ФАЙЛОМ (21.08.2026, задача владельца)
  //
   ...
   * тем же значением, и код, написанный как `const x = ctx.get('x'); if (x)`,
   * тихо уходит по ветке «этой возможности просто нет». Снаружи это выглядит
   * штатной работой: агент собирается, отвечает и молчит о том, чего лишился.
   *
   * Поймано трижды за три дня, каждый раз дороже предыдущего:
   *   19.08.2026 — фабрика агентов (см. withFactoryRetry выше);
   *   22.08.2026 — sessionPersistence: блок продолжения истории пропускался
   *                целиком, агент создавался поверх существующего журнала, и
   *                КАЖДЫЙ ход умирал с «id collision» за считаные миллисекунды;
   *   22.08.2026 — agentPresets: набор инструментов у модели упал с 33 до 3,
   *                без ошибки и без строки в журнале.
   * Общий вывод, ради которого функция ОДНА, а не три: лечение, применённое к
   * экземпляру, а не ко всем местам класса, гарантирует рецидив.
   *
   * ГРАНИЦА ОЖИДАНИЯ — назвать обязательно, иначе защита сама станет отказом:
   * ждём не дольше `tries` секунд и только при заведении агента. Не дождались —
   * говорим ГРОМКО, называем последствие и идём дальше. Канал не запирается там,
   * где сервиса в сборке нет вовсе (это законная сборка, а не поломка).
   */
  ...
  async function agentFor(chatId) {
    ...
    if (persistence) {
      ...
      try {
        ...
        log(`🔴 [${key}] не смог перечислить сохранённые сессии: ${e?.message ?? e}`);
      }
      // 🔴 УХОДИТЬ В create() МОЛЧА НЕЛЬЗЯ — ЭТО ВТОРОЙ ТИХИЙ ПУТЬ В ЭТОЙ ЖЕ
      // ФУНКЦИИ. Снаружи он неотличим от честного первого запуска, а на деле
      // означает create() поверх существующего журнала и «id collision» на
      // каждом ходе. Поэтому список сообщаем ЧИСЛОМ и именами.
      if (known !== undefined && !known.includes(sessionId)) {
        // Пустой список — честный первый запуск, тревожить нечем. Тревога там,
        // где сохранённые сессии ЕСТЬ, а нашей среди них нет.
