...
// 🔴 ВЕРСИЯ ЧИТАЕТСЯ ИЗ СВОЕГО МАНИФЕСТА, А НЕ ПИШЕТСЯ ЛИТЕРАЛОМ (долг 108, 04.09.2026).
// ЗАЧЕМ. Плагин давал 1648 строк за подъём и ни в одной не называл свою редакцию. Узнать её у
// работающего экземпляра было НЕЧЕМ: время файлов не годится (npm ставит всем файлам в тарболе
// одну метку 1985-10-26 08:15:00), а сумма на диске отвечает про диск, а не про то, что процесс
// прочитал при старте. Сегодня строка `[dsh-pamyat-core <версия>]` у соседнего пакета трижды
// закрывала спор о том, что в бою; здесь такой строки не было, и редакцию приходилось ВЫВОДИТЬ.
// ПОЧЕМУ НЕ ЛИТЕРАЛ: он разойдётся с манифестом молча — это наш класс парных объектов.
const VERSIYA = (() => {
  try { return createRequire(import.meta.url)('../package.json').version ?? 'неизвестна'; }
  catch { return 'неизвестна'; }   // 🔴 не «1.х»: выдуманное число хуже честного «не знаю»
})();
...
export function apply(ctx, config = {}) {
  ...
  // Первая строка подъёма называет РЕДАКЦИЮ — чтобы «что в бою» спрашивали у журнала,
  // а не выводили из времён файлов и сумм.
  log(`[dsh-telegram-multiagent ${VERSIYA}] подъём`);
  ...
  // Шина между агентами машины: отправитель кладёт письмо в щель, а получатель
  // забирает. Каталог задаётся настройкой spoolDir; у НАС это /var/spool/a2a/<агент>
  // (режим 1730: опустить можно, прочесть нельзя) — путь приведён как образец
  // раскладки, кодом он не прибит и у поставившего пакет будет свой,
