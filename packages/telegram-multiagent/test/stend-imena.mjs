...
// 🔴 ОБЛАСТЬ ПРОВЕРКИ — СТРОКОЙ В ВЫВОДЕ, А НЕ В ЧЬЕЙ-ТО ПАМЯТИ.
// Аудитор испортил комментарий в src/index.js — стенд промолчал, и это ГРАНИЦА,
// а не слепота: он никогда и не обходил весь предмет на имена. Но зелёный без
// названной области через полгода прочтётся как «пакет чист целиком».
console.log(`\nОБЛАСТЬ: имена как ДАННЫЕ ищутся только в самом стенде `
  + `(${Array.isArray(SPISOK_IMEN) ? SPISOK_IMEN.length : 0} имён из списка). `
  + `В предмете ${path.basename(SRC)} кличка проверяется ТОЛЬКО в двух строках — `
  + `тревога о чужом и приветствие. Остальной файл и остальные файлы пакета `
  + `на имена НЕ проверяются НИКЕМ — стенда по всему составу у этого пакета нет.`);
