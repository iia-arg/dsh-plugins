#!/usr/bin/env node
/**
 * Приёмка постановки цели из канала.
 *
...
try {
  ...
  // 🔴 НЕ сверяем с зашитым номером: такой стенд краснеет на КАЖДОМ выпуске и
  // его перестают читать. Проверяем форму — версия есть и она трёхчастная.
  /^\d+\.\d+\.\d+$/.test(String(pj.version || ''))
    ? pass('версия объявлена и имеет форму X.Y.Z', `got=${pj.version}`)
    : fail('версия объявлена и имеет форму X.Y.Z', `got=${pj.version}`);
...
try {
  ...
  // 🔴 Сверяем ЧИСЛО из кода с числом в README, а не дословную фразу: проверка
  // по формулировке ломается от любой переписки текста, и тогда стенд краснеет
  // на исправном предмете. Ищем число рядом со словом про час.
  // \b и \n в шаблонной строке JS — это backspace и перевод строки, а НЕ границы
  // регулярного выражения: их надо удваивать. Первая редакция этого не делала и
  // краснела на исправном README.
  const predelVReadme = new RegExp(`\\b${maxInCode}\\b[^.\\n]{0,60}час`, 'i').test(readme)
                     || new RegExp(`час[^.\\n]{0,60}\\b${maxInCode}\\b`, 'i').test(readme);
  predelVReadme
    ? pass('предел в README и в коде сходится', `код=${maxInCode}`)
    : fail('предел в README разошёлся с кодом', `код=${maxInCode}, в README не найден рядом со словом «час»`);
